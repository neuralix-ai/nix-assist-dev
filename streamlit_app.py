...
st.title("Nix-Assist Demo")
st.write("A streamlit demo of nix-assist feature requirements")

llm = ChatGoogleGenerativeAI(model="gemini-pro")

if "llm" not in st.session_state:
    st.session_state["llm"] = "gemini-pro"
...
if prompt := st.chat_input("How can I help?"):
    ...
    with st.chat_message("assistant"):
        stream = llm.stream(input=prompt)
        response_content = ""
        
        # Display stream of messages from the response
        for chunk in stream:
            response_content += chunk.content # chunk.get("content", "")
